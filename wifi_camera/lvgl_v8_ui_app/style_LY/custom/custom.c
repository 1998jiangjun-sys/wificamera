...
#define KEY_VOLUME_STEP_PERCENT    20
...
bool usb_flag = false;
static bool usb_boot_splash_done = false;

bool usb_boot_splash_is_done(void)
{
	return usb_boot_splash_done;
}

void usb_mark_boot_splash_done(void)
{
	usb_boot_splash_done = true;
}
bool update_date = false; //true时表示需要更新 rtc 时间，默认初始化时需要更新
extern char video_rec_car_num[64];
extern lv_ui guider_ui;
extern void ui_wifi_status_push(bool force);
extern void ui_wifi_start_polling(void);

static void shutdown_screen_show(void)
{
	if (ui_is_act_scr(GUI_SCREEN_SHUT_DOWN)) {
		return;
	}

	gui_scr_t *screen = ui_get_scr(GUI_SCREEN_SHUT_DOWN);
	if (!screen) {
		printf("[%s] failed to get shutdown screen\n", __func__);
		return;
	}
...
void key_scroll_to_view(void)
{
	...
	/* 增加空指针和对象有效性检查，避免在未聚焦或父对象为空时访问导致内存异常 */
	if (!focused_obj) {
		return;
	}

	lv_obj_t *focused_obj_parent = lv_obj_get_parent(focused_obj);
	if (!focused_obj_parent) {
		return;
	}

	/* 进一步检查对象是否仍然有效（避免已被删除的对象） */
...
static s16 key_volume_get_max_steps(void)
{
	static s16 s_cached_max_steps = -1;
	if (s_cached_max_steps <= 0) {
		s_cached_max_steps = app_audio_get_max_volume();
		if (s_cached_max_steps <= 0) {
			s_cached_max_steps = app_audio_volume_max_query(AppVol_BT_MUSIC);
		}
		if (s_cached_max_steps <= 0) {
			s_cached_max_steps = 100;
		}
		app_audio_set_max_volume(APP_AUDIO_STATE_MUSIC, s_cached_max_steps);
		printf("[Volume] key helper max steps: %d\n", s_cached_max_steps);
	}
	return s_cached_max_steps;
}

static u8 key_volume_steps_to_percent(s16 steps)
{
	s16 max_steps = key_volume_get_max_steps();
	if (steps <= 0) {
		return 0;
	}
	if (steps >= max_steps) {
...
static s16 key_volume_percent_to_steps(u8 percent)
{
	s16 max_steps = key_volume_get_max_steps();
	if (percent >= 100) {
		return max_steps;
...
static u8 key_volume_pick_active_state(void)
{
	...
	case APP_AUDIO_STATE_CALL:
		return state;
	default:
		return APP_AUDIO_STATE_MUSIC;
	}
}

static void key_volume_sync_ui_cb(int percent)
{
	if (percent < 0) {
		percent = 0;
	} else if (percent > 100) {
	...
	if (gui_scr_get_id(act) != GUI_SCREEN_VOLUME) {
		return;
	}
	...
	if (ui_scr->volume_lbl_3) {
		lv_label_set_text_fmt(ui_scr->volume_lbl_3, "%d%%", percent);
	}
}

static void key_volume_sync_ui(u8 percent)
{
	if (lvgl_rpc_post_func(key_volume_sync_ui_cb, 1, percent)) {
		printf("[Volume] failed to post slider sync, percent=%u\n", percent);
	}
}

static void key_photo_take_picture_async(void)
{
	int ret = app_send_message(APP_MSG_PHOTO_DELAY, 0);
	if (ret) {
		printf("[Photo] take photo message failed, ret=%d\n", ret);
	}
}

static void key_video_rec_toggle_async(void)
{
	video_rec_toggle_recording(&guider_ui, false);
}

static void key_ai_ident_async(void)
{
	const char *active_path = video_dir_landscape_get_active_path();
	if (!active_path || !video_dir_landscape_active_is_image()) {
		printf("[AI识图] KEY_F1 ignored: no active image\n");
		return;
	}
	...
	if (screen) {
		...
		printf("[AI识图] KEY_F1: AI screen not available\n");
	}
}

static void key_volume_apply_delta(int delta_percent)
{
	u8 active_state = key_volume_pick_active_state();
	u8 query_state = active_state;

	struct application *app = __get_current_app();
	printf("[Volume] current app: %s, state=%u\n", app ? app->name : "<null>", active_state);

	if (active_state == APP_AUDIO_STATE_FLOW) {
		/* FLOW 共享音乐音量表，读取当前音量仍按音乐声道 */
		query_state = APP_AUDIO_STATE_MUSIC;
	}

	s16 current_steps = app_audio_get_volume(query_state);
	if (current_steps < 0) {
		current_steps = 0;
	}

	u8 current_percent = key_volume_steps_to_percent(current_steps);
	int target_percent = current_percent + delta_percent;
	...
	s16 target_steps = key_volume_percent_to_steps((u8)target_percent);
	if (target_steps == current_steps) {
		printf("[Volume] key delta %d%% ignored (at %d%%)\n", delta_percent, current_percent);
		return;
	}

	app_audio_set_volume_def_state(0);
	app_audio_set_volume(active_state, target_steps, 1);
	printf("[Volume] key set to %d%% (step %d/%d, state %u)\n", target_percent, target_steps, key_volume_get_max_steps(), active_state);
	key_volume_sync_ui((u8)target_percent);
}

int key_event_remap(struct key_event *e)
{
	printf("=====>%s()--->key action: %d, value: %d\n", __func__, e->action, e->value);
	static bool pwr_key_longpress = false;
	static bool photo_key_pressed = false;
	static bool ai_key_pressed = false;
	if ((e->value == KEY_UP) || (e->value == KEY_DOWN)) {
		int delta_percent = (e->value == KEY_UP) ? KEY_VOLUME_STEP_PERCENT : -KEY_VOLUME_STEP_PERCENT;

		switch (e->action) {
		case KEY_EVENT_DOWN:
			key_volume_apply_delta(delta_percent);
			break;
		case KEY_EVENT_LONG:
		case KEY_EVENT_HOLD:
			key_volume_apply_delta(delta_percent);
	...
	if (e->value == KEY_PHOTO) {
		switch (e->action) {
		case KEY_EVENT_DOWN:
			photo_key_pressed = true;
			break;
		case KEY_EVENT_UP:
			if (photo_key_pressed) {
				photo_key_pressed = false;
				gui_scr_t *act = gui_scr_get_act();
				if (!act) {
					break;
				}
				int32_t scr_id = gui_scr_get_id(act);
				if (scr_id == GUI_SCREEN_VIDEO_PHOTO) {
					printf("[Photo] KEY_PHOTO released -> take photo\n");
					if (lvgl_rpc_post_func((void (*)(void *, ...))key_photo_take_picture_async, 0)) {
						key_photo_take_picture_async();
					}
					return false;
				} else if (scr_id == GUI_SCREEN_VIDEO_REC) {
					printf("[Video Rec] KEY_PHOTO released -> toggle recording\n");
					if (lvgl_rpc_post_func((void (*)(void *, ...))key_video_rec_toggle_async, 0)) {
						key_video_rec_toggle_async();
					}
					return false;
				}
			}
	...
	if (e->value == KEY_F1) {
		...
		case KEY_EVENT_UP:
			if (ai_key_pressed) {
				ai_key_pressed = false;
				gui_scr_t *act = gui_scr_get_act();
				if (!act) {
					break;
				}
				int32_t scr_id = gui_scr_get_id(act);
				if (scr_id == GUI_SCREEN_VIDEO_DIR_LANDSCAPE_MODE) {
					printf("[AI识图] KEY_F1 released -> start identification\n");
					if (lvgl_rpc_post_func((void (*)(void *, ...))key_ai_ident_async, 0)) {
						key_ai_ident_async();
					}
					return false;
				}
	...
	if (e->value == KEY_POWER) {
		...
		case KEY_EVENT_LONG:
		case KEY_EVENT_HOLD:
			if (!pwr_key_longpress) {
				pwr_key_longpress = true;
				if (lvgl_rpc_post_func((void (*)(void *, ...))shutdown_screen_show, 0)) {
					pwr_key_longpress = false;
				}
			}
			break;
		case KEY_EVENT_UP:
...
static void clear_persistent_wifi_data(void)
{
#if defined(CONFIG_NET_ENABLE) && defined(CONFIG_WIFI_ENABLE)
	/* 清除历史保存的STA连接信息 */
	struct wifi_stored_sta_info blank_sta = {0};
	for (u16 id = WIFI_STA_INFO_IDX_START; id < WIFI_STA_INFO_IDX_END; ++id) {
		int write_len = syscfg_write(id, &blank_sta, sizeof(blank_sta));
		if (write_len != sizeof(blank_sta)) {
			printf("[Factory Reset] WARN: failed to wipe STA slot %u (ret=%d)\n", id, write_len);
		}
	}

	/* 清除基础Wi-Fi配置并同步运行时缓存 */
	struct wifi_store_info blank_info = {0};
	int info_len = syscfg_write(WIFI_INFO_IDX, &blank_info, sizeof(blank_info));
	if (info_len != sizeof(blank_info)) {
		printf("[Factory Reset] WARN: failed to wipe WIFI_INFO (ret=%d)\n", info_len);
	}
	int ext_len = syscfg_write(EXT_WIFI_INFO_IDX, &blank_info, sizeof(blank_info));
	if (ext_len != sizeof(blank_info)) {
		printf("[Factory Reset] WARN: failed to wipe EXT_WIFI_INFO (ret=%d)\n", ext_len);
	}

	char empty_ssid[1] = {0};
	char empty_pwd[1] = {0};
	int store_res = wifi_store_mode_info(STA_MODE, empty_ssid, empty_pwd);
	if (store_res) {
		printf("[Factory Reset] WARN: wifi_store_mode_info clear failed (%d)\n", store_res);
	}

	/* 清除扫描结果、PMK、BSS表和IP地址等缓存数据 */
	syscfg_write(WIFI_SCAN_INFO, NULL, 0);
	syscfg_write(WIFI_PMK_INFO, NULL, 0);
	syscfg_write(WIFI_BSS_TABLE, NULL, 0);
	syscfg_write(VM_STA_IPADDR_INDEX, NULL, 0);

	printf("[Factory Reset] cleared stored WiFi information.\n");
#endif
}
...
void custom_init(lv_ui *ui)
{
    /* Add your codes here */
    printf("[chili] %s %d   \n", __func__, __LINE__);
	ui_battery_start_polling();
	ui_wifi_status_push(true);
	ui_wifi_start_polling();
}
...
#if defined(CONFIG_NET_ENABLE) && defined(CONFIG_WIFI_CAMERA_WEBSOCKET_ENABLE)
	int shutdown_report_err = wifi_camera_ws_send_shutdown_report();
	if (shutdown_report_err) {
		printf("[WS][RESET] shutdown report failed:%d\n", shutdown_report_err);
	} else {
		printf("[WS][RESET] shutdown report sent\n");
	}
#endif

	clear_persistent_wifi_data();

	reset_err = db_reset();
	//ui_factory_reset_runtime_state();
