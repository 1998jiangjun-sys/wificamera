...
extern void ui_home_request_skip_back_once(void);
...
void usb_page_show(int arg)
{
    ...
    if (arg == 0 && usb_boot_splash_is_done()) {
        printf("[usb] boot splash already completed, skip showing usb slave screen\n");
        return;
    }
...
void usb_page_hide(int arg)
{
    ...
    if (usb_flag) {
        usb_flag = false;
        ui_home_request_skip_back_once();
