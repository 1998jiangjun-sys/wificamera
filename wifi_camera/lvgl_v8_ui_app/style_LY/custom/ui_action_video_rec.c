...
int gui_src_action_video_rec(int action)
{
    ...
    case GUI_SCREEN_ACTION_UNLOAD:
        ...
        app_mode_change_replace(APP_MODE_SYSTEM);
        key_event_enable();
        break;
    }
...
int video_rec_key_handler(struct key_event *key)
{
    ...
    case KEY_EVENT_CLICK:
        ...
        case KEY_UP:
            ...
            // if (!lv_obj_has_flag(ui_scr->video_rec_view_menu_b, LV_OBJ_FLAG_HIDDEN)) 
            // {  //进入设置菜单时, 按键消息不传到792 app_core
            //     return 0;
            // } else 
            {
                return 1;
...
void video_rec_post_msg(const char *msg, ...)
{
    ...
    if (strstr(msg, "Remain")) {
        ...
        int valid = (value >= 0 && value <= 100);
        ui_battery_apply_percent(value, valid);
    } else {
