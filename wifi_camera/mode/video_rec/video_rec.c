...
#if !defined(VIDEO_REC_DEFAULT_ROTATE)
#define VIDEO_REC_DEFAULT_ROTATE ((SCREEN_W < SCREEN_H) ? 90 : 0)
#endif

#if !defined(VIDEO_REC_DEFAULT_MIRROR)
#define VIDEO_REC_DEFAULT_MIRROR (ROTATE_NEED_SWAP(VIDEO_REC_DEFAULT_ROTATE) ? VIDEO_VER_MIRROR : VIDEO_DIR_NONE)
#endif
...
printf("video_disp_start:id=%d,w=%d,h=%d\n", id, win->width, win->height);
    if (win->width == (u16) - 1) {
    ...
    int full_screen_window = 0;
    u32 display_rotate = 0;
    u16 display_width = win->width;
    u16 display_height = win->height;
    u16 display_left = win->left;
    u16 display_top = win->top;
    int online_nums = video_rec_online_nums();
    int matches_screen = (win->width == SCREEN_W && win->height == SCREEN_H);
    int matches_swapped = (win->width == SCREEN_H && win->height == SCREEN_W);

    if (matches_screen || matches_swapped || (online_nums <= 1)) {
        full_screen_window = 1;
        display_rotate = VIDEO_REC_DEFAULT_ROTATE;
    ...
    req.display.left  	        = display_left;
    req.display.top 	        = display_top;
    req.display.width 	        = display_width;
    ...
    req.display.rotate          = display_rotate;
    req.display.mirror          = full_screen_window ? VIDEO_REC_DEFAULT_MIRROR : win->mirror;
