...
extern int fb_combine_layer_modify(u8 id, const char *fb_name, struct fb_map_user *map);
...
#define VIDEO_DEC_ROTATE_ANGLE            270
...
static u8 video_fb_rotation_applied = 0;

static void video_dec_apply_fb_rotation(void)
{
    struct fb_map_user map = {0};

    if (!__this->req.dec.fb) {
        return;
    }

    if ((VIDEO_DEC_ROTATE_ANGLE % 360) == 0) {
        video_fb_rotation_applied = 0;
        return;
    }

    map.width = __this->req.dec.width;
    map.height = __this->req.dec.height;
    map.real_width = __this->req.dec.width;
    map.real_height = __this->req.dec.height;
    map.rotate = VIDEO_DEC_ROTATE_ANGLE;
    map.mirror = 0;
    map.xoffset = 0;
    map.yoffset = 0;

    fb_combine_layer_modify(LCD_ID, __this->req.dec.fb, &map);
    video_fb_rotation_applied = 1;
}

static void video_dec_reset_fb_rotation(void)
{
    struct fb_map_user map = {0};

    if (!video_fb_rotation_applied || !__this->req.dec.fb) {
        return;
    }

    map.width = LCD_W;
    map.height = LCD_H;
    map.real_width = LCD_W;
    map.real_height = LCD_H;
    map.rotate = 0;
    map.mirror = 0;
    map.xoffset = 0;
    map.yoffset = 0;
...
static int video_dec_start(void *p)
{
    ...
    __this->req.dec.rotate = VIDEO_DEC_ROTATE_ANGLE;
    if (ROTATE_NEED_SWAP(__this->req.dec.rotate)) {
        __this->req.dec.width 	= LCD_H;
        __this->req.dec.height 	= LCD_W;
    } else {
