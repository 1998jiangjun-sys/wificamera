...
static unsigned int user_read_power_key(int dly)
{
    gpio_direction_input(TCFG_LOWPOWER_WAKEUP_PORT0_IO);
    gpio_set_die(TCFG_LOWPOWER_WAKEUP_PORT0_IO, 1);
    gpio_set_pull_up(TCFG_LOWPOWER_WAKEUP_PORT0_IO, 0);
    gpio_set_pull_down(TCFG_LOWPOWER_WAKEUP_PORT0_IO, 0);

    if (dly) {
        delay_us(3000);
    }

    return (gpio_read(TCFG_LOWPOWER_WAKEUP_PORT0_IO));
}
...
#if 0 //TCFG_PRESS_LONG_KEY_POWERON_ENABLE
...
#endif
#if TCFG_LOWPOWER_WAKEUP_PORT0_ENABLE
    wake_keyval = is_gpio_wakeup(TCFG_LOWPOWER_WAKEUP_PORT0_IO);
    printf("11111666wake_keyval=%d\n", wake_keyval);
    if (wake_keyval) {
        for (int i = 0; i < 500; i++) {
            if (user_read_power_key(1)) {
                //sys_power_poweroff();
                break;
            }
        }
...
#endif
#endif
gpio_direction_output(TCFG_AUDIO_DAC_PA_MUTE_PORT, 1);
